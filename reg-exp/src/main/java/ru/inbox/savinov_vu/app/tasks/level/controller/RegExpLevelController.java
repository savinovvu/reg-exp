...
public class RegExpLevelController {
  ...
  public ResponseEntity<List<RegExpLevel>> getAll(HttpServletRequest request, Principal principal) {
    ...
    return new ResponseEntity(regExpLevelService.findAll(user.getId()), HttpStatus.ACCEPTED.OK);
  }


  @GetMapping("/v1/tasks/regexplevel/{id}")
  public ResponseEntity<RegExpLevel> getById(@PathVariable("id") Integer id) {
    return new ResponseEntity(regExpLevelService.findById(id), HttpStatus.ACCEPTED.OK);
  }
  ...
  public ResponseEntity<RegExpLevel> getByNumber(@PathVariable("number") Integer number) {
    return new ResponseEntity(regExpLevelService.findByNumber(number), HttpStatus.ACCEPTED.OK);
  }


  @PostMapping("/v1/tasks/regexplevel")
  public ResponseEntity<RegExpLevel> add(RegExpLevel regExpLevel) {
    return new ResponseEntity(regExpLevelService.create(regExpLevel), HttpStatus.ACCEPTED.OK);
  }
